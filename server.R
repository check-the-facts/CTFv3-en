...
server <- function(input, output, session) {
  ...
  update_all <- function(x) {
    updateSelectInput(session, "tab",
                      choices = c("", "Study Programs", "Job market", "Analytics", "Sources"),
                      label = "",
                      selected = x
    )
  }
  ...
  observeEvent(input$jobmarket, {
    update_all("Job market")
  })
  ...
  observeEvent(input$tab, {
    ...
    updateButton(session, "Job market", style = {
      if (x == "Job market") {
        paste("warning")
      } else {
        paste("primary")
  ...
  output$ins_stream <- renderStreamgraph({
    ...
    if(input$streamin == 'INSTELLINGSNAAM'){
      ...
      data4 %>%
        ...
        filter(OPLEIDINGSVORM %in% input$fullpartInput)  %>%
        filter(INSTELLINGSNAAM %in% input$instituteInput) %>%
        select(OPLEIDINGSNAAM.ACTUEEL, Year, Registered) %>%
        group_by(OPLEIDINGSNAAM.ACTUEEL, Year) %>%
        summarise_all(funs(sum)) %>%
        streamgraph(key="OPLEIDINGSNAAM.ACTUEEL", value="Registered", date="Year", height="300px", width="1000px") %>%
        sg_axis_x(1)
  ...
  output$selected_var_stream <- renderText({ 
    paste("The number of registrations from 2015 to 2019 per ", input$streamin)
  })
  
  output$selected_var_bar <- renderText({
    paste("The number of registrations per",input$barin ,"in", input$YearInput)
  })
  ...
  output$selected_var_job <- renderText({
    paste("The number of alumni per",input$jobin ,"in", input$jobyearInput)
  })
